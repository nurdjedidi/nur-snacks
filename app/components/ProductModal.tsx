...
export default function ProductModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md transition-all duration-300 animate-fade-in">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col md:flex-row max-h-[90vh] md:max-h-[80vh] z-10 animate-scale-up">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 transition z-20 cursor-pointer"
          title="Fermer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className={`w-full md:w-1/2 flex items-center justify-center p-6 ${product.bgClass} ${product.colorClass} border-b md:border-b-0 md:border-r border-slate-100 min-h-[220px] md:min-h-auto relative`}>
          {!imgError ? (
            <img
              src={product.imagePath}
              alt={product.name}
              className="max-h-[200px] md:max-h-[300px] w-auto object-contain transition-transform duration-500 hover:scale-105"
              onError={() => setImgError(true)}
            />
          ) : (
            <CategoryIcon className="w-24 h-24 stroke-[1] select-none" />
          )}
        ...
        <div className="w-full md:w-1/2 p-6 flex flex-col justify-between overflow-y-auto">
          <div>
            ...
            <h2 className="text-2xl font-black text-slate-950 mt-1 leading-tight">{product.name}</h2>
            
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-2xl font-black text-amber-500">{product.price.toFixed(2)}€</span>
              <span className="text-xs text-slate-500 font-bold">{product.unit}</span>
            </div>

            <p className="mt-4 text-xs leading-relaxed text-slate-700 border-b border-slate-100 pb-4">
              {product.description}
            </p>
            ...
            {/* Tab Contents */}
            <div className="mt-4 max-h-[150px] overflow-y-auto text-xs leading-relaxed text-slate-600">
              {activeTab === "ingredients" && (
